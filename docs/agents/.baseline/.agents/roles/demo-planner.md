# Shared role: demo-planner

Read the active platform adapter and `.agents/platforms/capabilities.md` before following this contract. Capability tokens below describe operations, not literal tool names.

# Source Mapping

## Bootstrap Template Knowledge Source

The knowledge index is derived from `templates/knowledge-index-schema.md`; use `docs/agents/knowledge/README.md` to select knowledge before reading individual documents.

`docs/agents/plan-schema.md` takes precedence over Markdown cleanup for required filesystem-tree links, File Details anchors, backlinks, approval metadata, operations, validation, and risks.
...
Use the repository-search binding in the active platform adapter. Keep discovery bounded and use only available tools permitted for this role.

- Use `the active platform repository-search binding` for repository discovery when planning requires codebase evidence.
...
| `#capability:work-item-retrieval`        | Use the GitHub read binding in `.agents/platforms/capabilities.md` for the requested External Issue ID.                                                         |
| `#capability:work-item-type-retrieval`   | Use the GitHub read binding in `.agents/platforms/capabilities.md` to determine the work item type.                                                             |
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Work-item tracker access     | `#capability:work-item-retrieval`, `#capability:work-item-type-retrieval`                                                                                                                                                                                                                                                                                                                                                                                                                  | Read issue, story, type, or comment evidence. Follow `.agents/platforms/capabilities.md`; no local tracker fallback is approved.                                                                     |
...
1. Use the active platform delegation binding to invoke `demo-vision` for the following evidence task. If delegation is unavailable, use approved visual tools inline; if visual evidence cannot be inspected, ask for textual evidence and stop image-dependent planning.
