...
Use `the active platform repository-search binding` for codebase discovery when analysis requires repository evidence.
Read relevant project knowledge through `docs/agents/knowledge/README.md and its selected knowledge documents` before analysis when repository knowledge is available.
