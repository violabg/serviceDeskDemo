...
| Customer | An external service-desk contact associated with tickets. A Customer has contact details and can have many tickets. | Use `Customer` in application code rather than `Client`. | `prisma/schema.prisma` `Customer` model |
