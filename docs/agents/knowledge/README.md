...
| K5 | [dashboard-section-permission-flow-checklist.md](./dashboard-section-permission-flow-checklist.md) | Section permission flow | Before adding a new ACCESS_SECTIONS entry for a dashboard/admin section or gateway boundary and wiring its routes or navigation | The task changes only nested routes within an existing section, or does not add a section or permission boundary | Privilege-key creation, Admin role and configured administrator assignment, navigation eligibility, access gating rules |
