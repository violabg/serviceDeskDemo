...
| K9 | [session-artifact-workflow-checklist.md](./session-artifact-workflow-checklist.md) | Session artifact workflow | Before creating, repairing, approving, linting, or handing off session artifacts, implementation plans, clarification questions, approval metadata, or handoff envelopes | The task only changes app behavior, UI, database, runtime config, or product tests with no session-artifact workflow impact | Session activation, approval metadata, plan schema, clarification, handoff envelope, artifact lint rules |
| K10 | [access-control-auth-flow-checklist.md](./access-control-auth-flow-checklist.md) | Access control and auth flow | Before changing login, pending-access, dashboard access gates, role or permission bootstrap, local user sync, or permission-filtered service desk navigation | The task is limited to unrelated UI styling, non-auth utilities, or session-artifact workflow docs | Neon auth to local user sync, redirect order, permission ownership, seed and navigation rules |
| K11 | [ticket-domain-service-checklist.md](./ticket-domain-service-checklist.md) | Ticket domain and mutation rules | Before changing ticket Prisma models, ticket services, ticket actions, ticket pages, SLA logic, duplicate detection, or ticket cache invalidation | The task does not touch ticket workflows, ticket data, or ticket route behavior | Ticket invariants, mutation side effects, permission thresholds, cache-tag rules |
