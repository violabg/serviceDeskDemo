...
| K5 | [dashboard-section-permission-flow-checklist.md](./dashboard-section-permission-flow-checklist.md) | Section permission flow | Before adding a database entity needing independent privileges or a new ACCESS_SECTIONS boundary and wiring its routes or navigation | A new entity or nested route uses only an existing section and does not need a separate access boundary | Privilege-key creation, Admin role grant and configured administrator assignment, navigation eligibility, access gating rules |
...
| K7 | [nextjs-cache-components-pattern.md](./nextjs-cache-components-pattern.md) | Cache component pattern | When adding or refactoring pages, Server Action invalidation, or instant-navigation behavior involving caching or Suspense | No cache, invalidation, skeleton, or Suspense behavior changes are in scope | Caching boundaries, shared tag invalidation, read-your-writes, access and Suspense boundaries |
...
| K9 | [access-control.md](./access-control.md) | Access-control reference | Before changing an existing role, permission assignment, read/write/manage policy, or diagnosing seed and effective-permission behavior | The task only adds a new ACCESS_SECTIONS boundary and does not change existing role assignments; read K5 instead | Code-defined base permissions, operation gates, role assignment boundaries |
