...
agents: [agent, demo-vision.agent.md]
...
- Use `docs/agents/templates/question-schema.md` for blocking clarification questions and answer records.
...
- Load `docs/agents/templates/plan-schema.md` immediately before drafting or repairing `implementation-plan.md`.
...
2. Otherwise, use the user's requirement key phrase, normalized to an alphanumeric underscore format.
   ...
   Visual evidence support is installed through `demo-vision.agent.md`.
   When screenshots, mockups, diagrams, browser captures, or Figma-derived screenshots materially affect planning, invoke the generated Vision agent and use its artifact as deterministic planning input.

### IMAGE_INTAKE_INSTRUCTION

For every provided screenshot:

1. Invoke the subagent `#tool:agent/runSubagent` using `demo-vision.agent.md` with the following prompt template:
