...
Read relevant integration-test knowledge through `docs/agents/knowledge/README.md` and its selected knowledge documents before drafting project-specific test guidance.
Document the integration-test scope and out-of-scope boundaries: `Cover Next.js entry points and internal wiring with Vitest while stubbing third-party boundaries; exclude unit-only logic, browser end-to-end tests, and live Neon, Auth, or GitHub services`.
Document the target repository validation commands: `pnpm agent:lint-artifacts --mode <gate> --session <id>`, then `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` for buildable app changes.
...
- The knowledge names Vitest as the test framework.
- The knowledge names Testing Library and `@testing-library/jest-dom` as the assertion stack.
- The knowledge identifies any mocking approach from existing tests before adding one.
- The knowledge identifies any standard fixture or setup helper from existing tests before using it.
- The knowledge documents the Node test environment and cleanup after each test from `test/setup.ts`.
