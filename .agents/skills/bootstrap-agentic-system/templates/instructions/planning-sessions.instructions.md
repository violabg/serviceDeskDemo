...
- Retrieve the current issue only. Read every issue explicitly referenced or linked by the current issue through the approved tracker adapter, record each issue and its retrieval reason as dependency evidence, and do not decide relevance before retrieval. Do not recursively follow references.
